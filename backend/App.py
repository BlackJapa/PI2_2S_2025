...
def register():
    ...
    try:
        ...
        # Usando aspas duplas para forçar o PostgreSQL a respeitar as maiúsculas do Supabase
        cursor.execute('''
        ...
        return jsonify({'error': f'Bloqueio do BD: {erro_real}'}), 400
...
# --- Rotas de Blocos e Apartamentos ---
    ...
    try:
        ...
        if role == 'sindico':
            ...
            cursor.execute('''
                ...
                JOIN "Morador_Apartamentos" ma ON a.apartamento_id = ma.apartamento_id 
                WHERE ma.morador_id = %s LIMIT 1
            ''', (user_id,))
            res = cursor.fetchone()
...
def change_user_role(user_id):
    ...
    try:
        ...
        if new_role == 'admin_bloco':
            ...
            if not result:
                return jsonify({'error': 'Morador não encontrado ou sem apartamento'}), 404
